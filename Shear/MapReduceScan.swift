...
public extension TensorProtocol {
    ...
    public func map<A>(_ transform: (Element) throws -> A) rethrows -> Tensor<A> {
        let buffer = try self.allElements.map(transform)
        return Tensor(shape: shape, values: buffer)
    }
    
    /// Maps a `transform` upon each element of the TensorProtocol returning an TensorProtocol of the same shape with the results.
    public func map<A>(_ transform: @escaping (Element) -> A) -> Tensor<A> {
    ...
    public func vectorMap<A>(byRows rowVector: Bool = true, transform: ([Element]) throws -> [A]) rethrows -> Tensor<A> {
        ...
        if let first = slice.first, first.isScalar { // Slice is a [TensorSlice<Element>], we need to know if its constituent Tensors are themselves scalar.
    ...
    /// Maps a `transform` upon a vector of elements from the TensorProtocol. Either by rows (that is, row vectors of the column-seperated elements) or vectors of first-deminsion-seperated elements.
    public func vectorMap<A>(byRows rowVector: Bool = true, transform: @escaping ([Element]) -> [A]) -> Tensor<A> {
        let enclosed = enclose(rowVector ? [rank-1] : [0]).map { transform(Array<Element>($0.allElements)).ravel() }
    ...
    public func reduce<A>(_ initial: A, combine: @escaping ((A, Element)-> A)) -> Tensor<A> {
        return vectorMap(byRows: true, transform: {[$0.reduce(initial, combine)]})
    }
    
    /// Applies the `combine` upon the last axis of the TensorProtocol; returning an TensorProtocol with the last element of `self`'s shape dropped.
    public func reduce(_ combine: @escaping (Element, Element) -> Element) -> Tensor<Element> {
    ...
    public func scan<A>(_ initial: A, combine: @escaping (A, Element) -> A) -> Tensor<A> {
        return vectorMap(byRows: true, transform: {$0.scan(initial, combine: combine)})
    }
    
    /// Applies the `combine` upon the last axis of the TensorProtocol, returning the partial results of it's appplication.
    public func scan(_ combine: @escaping (Element, Element) -> Element) -> Tensor<Element> {
    ...
    public func reduceFirst<A>(_ initial: A, combine: @escaping ((A, Element)-> A)) -> Tensor<A> {
        return vectorMap(byRows: false, transform: {[$0.reduce(initial, combine)]})
    }
    
    /// Applies the `combine` upon the first axis of the TensorProtocol; returning an TensorProtocol with the first element of `self`'s shape dropped.
    public func reduceFirst(_ combine: @escaping (Element, Element) -> Element) -> Tensor<Element> {
    ...
    public func scanFirst<A>(_ initial: A, combine: @escaping (A, Element) -> A) -> Tensor<A> {
        return vectorMap(byRows: false, transform: {$0.scan(initial, combine: combine)})
    }
    
    /// Applies the `combine` upon the first axis of the TensorProtocol, returning the partial results of it's appplication.
    public func scanFirst(_ combine: @escaping (Element, Element) -> Element) -> Tensor<Element> {
