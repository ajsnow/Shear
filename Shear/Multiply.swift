...
private func elementwiseArrayProduct < A: Array, B: Array where A.Generator.Element == B.Generator.Element, A.Generator.Element: NumericType >
...
private func elementwiseScalarProduct < A: Array, Scalar: NumericType where A.Generator.Element == Scalar >
    (a: A, scalar: Scalar) -> DenseArray<A.Generator.Element> {
        let multiplyScalar = { $0 * scalar}
        return DenseArray(shape: a.shape, baseArray: a.map(multiplyScalar))
}

private func innerProduct < A: Array, B: Array where A.Generator.Element == B.Generator.Element, A.Generator.Element: NumericType >
    (lhs: A, _ rhs: B) -> DenseArray<A.Generator.Element> {
